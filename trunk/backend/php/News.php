...
class News {
            ...
            if($param['v1'] == "lire_news" && isset($param['v2'])) {
                ...
                if ( isset($param['news_com']) && $param['news_com'] == 'true') {
                    if($this->add_com_byNewsId($_POST['id_news'], $_POST['message'])){
                        ...
                        header("Location:index.php?page=err&msg=Erreur");
                    }
                }
            } else if($param['v1'] == "research_news" && isset($param['v2']))  {
                $this->get_news_like($param['v2']);
            } else {
                $this->get_all_news();
            }
        } else {
     ...
     */
    public function get_news_like($val) {
        $val = $val."%";
    ...
    /*
      \brief supprime la news en fonction de son Id (statique)
      \param id l'identifiant de la news
      \return elle retourne vrai si la suppression a reussi faux sinon
     */
    static public function s_delete_byId($bdd, $id) {
        $query = $bdd->prepare("delete  from NEWS where Id = :id");
	$query2 = $bdd->prepare("delete from NEWS_COM where idNews = :id"); 
        $query->bindParam(":id", $id);
