            ...
            if(isset($_FILES)) {
                ...
                if($nb_files > 0) {
                    ...
                    for($i = 0; $i < $nb_files; $i++) {
                        $path .= "/";
                        if(move_uploaded_file($_FILES['photo']['tmp_name'][$i], $path.$_FILES['photo']['name'][$i])) {
                            $bdd->query("insert into PHOTO values(null, '".$_FILES['photo']['name'][$i]."', '../photos/tournois/".$_POST['id_match']."/".$_FILES['photo']['name'][$i]."', 'aucune')");
                            $id_photo = $bdd->query("select MAX(Id) as id_max from PHOTO")->fetch()['id_max'];
                            $bdd->query("insert into PHOTO_TOURNOI values(".$_POST['id_tournoi'].", ".$id_photo.")");
                        }
                    }
